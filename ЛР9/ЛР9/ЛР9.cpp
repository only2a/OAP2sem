...
//-----------------------------------------------------------
void addXEnd(Address* e, Address** phead, Address** plast) //Добавление в конец списка
{
	Address* p = *plast;
	if (*plast == NULL)
	{
...
void readFromFile(Address** phead, Address** plast)          //Считывание из файла
{
	...
	*phead = *plast = NULL;
	...
	while (!feof(fp))
	{
		...
		addXEnd(t, phead, plast);
	}
	fclose(fp);
}
//-----------------------------------------------------------
void DeleteX(Address* t, char word) {
...
int main(void)
{
	...
	while (true)
	{
		switch (menu())
		{
		case 1:  addXEnd(setElement(), &head, &last);
			break;
